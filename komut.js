...
class Komut {
    constructor(client, msg, args) {
        ...
        this.reklamengelle = this.constructor.reklamengelle;
      this.küfürengelle = this.constructor.küfürengelleme;
...
this.message.channel.send(atasozu)   
  ...
  static async reklamengelle(args) {
  if(!args) this.message.channel.send("Reklam engelle aç yada kapat yaz.")
   if (!this.message.member.hasPermission('MANAGE_GUILD')) return this.message.channel.send('Olamaz! "Sunucuyu Yönet" yetkin yok!')
  
  if (args[0] == 'aç') {
    db.set(`reklam_${this.message.guild.id}`, 'acik')
      this.message.channel.send(':white_check_mark: Reklam Engel başarıyla açıldı! Üyeleri Yasakla yetkisine sahip olanlar`ın reklama engellenmicektir.')
    
  }
  if (args[0] == 'kapat') {
    db.set(`reklam_${this.message.guild.id}`, 'kapali')
      this.message.channel.send(':white_check_mark: Reklam Engel başarıyla kapatıldı! Artık herkes reklam yapabilir.')
    }
  }

 static async küfürengelle(args) {
   if(!args) this.message.channel.send("Küfür engelle aç yada kapat yaz.")
    if (!this.message.member.hasPermission('MANAGE_GUILD')) return this.message.channel.send('Olamaz! "Sunucuyu Yönet" yetkin yok!')
  
  if (args[0] == 'aç') {
    db.set(`kufur_${this.message.guild.id}`, 'acik')
      this.message.channel.send(':white_check_mark: Küfur Engel başarıyla açıldı! Üyeleri Yasakla yetkisine sahip olanların küfürü engellenmicektir.')
    
  }
  if (args[0] == 'kapat') {
    db.set(`kufur_${this.message.guild.id}`, 'kapali')
      this.message.channel.send(':white_check_mark: Küfür Engel başarıyla kapatıldı! Artık herkes küfür yazabilir.')
